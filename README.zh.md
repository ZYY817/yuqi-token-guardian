...
**在 DeepSeek 官方接口实测：同一个任务里，模型把自己已经想过的内容重复发送了 165 万个字符 —— 每一轮请求都在为旧思考重新付费。装上这个插件后，重复发送降到 2.3 万字符。**
...
```
plugins/token-guardian.js   编译产物，单文件零依赖（装这个）
plugins/token-guardian.ts   TypeScript 源码
plugins/metrics-logger.*    可选观测插件（需自配 out 路径）
